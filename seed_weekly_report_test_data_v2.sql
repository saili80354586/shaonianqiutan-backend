...
-- 1. 创建测试球员账号（8名球员，使用2001开头的手机号避免冲突）
...
-- U12一队球员 (4名)
(1001, '13800221001', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '王小明', '王小明', 'user', 'active', '男', 11, '前锋', '右脚', datetime('now'), datetime('now')),
(1002, '13800221002', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '李小强', '李小强', 'user', 'active', '男', 12, '中场', '左脚', datetime('now'), datetime('now')),
(1003, '13800221003', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '张小刚', '张小刚', 'user', 'active', '男', 11, '后卫', '右脚', datetime('now'), datetime('now')),
(1004, '13800221004', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '刘小军', '刘小军', 'user', 'active', '男', 12, '守门员', '右脚', datetime('now'), datetime('now')),
-- U12二队球员 (2名)
(1005, '13800221005', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '陈小龙', '陈小龙', 'user', 'active', '男', 11, '前锋', '左脚', datetime('now'), datetime('now')),
(1006, '13800221006', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '赵小虎', '赵小虎', 'user', 'active', '男', 12, '中场', '右脚', datetime('now'), datetime('now')),
-- U14精英队球员 (2名)
(1007, '13800221007', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '马小军', '马小军', 'user', 'active', '男', 13, '前锋', '右脚', datetime('now'), datetime('now')),
(1008, '13800221008', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjIXdB5YW8WfwcPx9DYRH0XmDnn0gS.', '周小杰', '周小杰', 'user', 'active', '男', 14, '后卫', '左脚', datetime('now'), datetime('now'));

-- 2. 创建球队-球员关联
DELETE FROM team_players WHERE user_id >= 1001 AND user_id <= 1008;
...
-- 4. 删除旧的周报数据（避免冲突）
DELETE FROM weekly_reports WHERE id BETWEEN 1 AND 20;
DELETE FROM weekly_report_periods WHERE id BETWEEN 1 AND 10;

-- 5. 创建周报周期（当前周和上周）
INSERT INTO weekly_report_periods (id, team_id, week_start, week_end, deadline, status, total_players, submitted_count, pending_count, reviewed_count, created_at, updated_at) VALUES
-- 上周（已归档）
...
-- 6. 创建周报数据（多种状态，形成完整测试场景）

-- ===== 上周数据（已归档） =====
-- 王小明 - 已通过
INSERT INTO weekly_reports (id, team_id, player_id, coach_id, week_start, week_end, deadline, submit_status, review_status,
...
-- 7. 创建一些通知数据（让通知中心有内容）
DELETE FROM notifications WHERE user_id >= 1001 AND user_id <= 1008;
INSERT INTO notifications (user_id, type, title, content, is_read, priority, created_at) VALUES
-- 给王小明的通知
...
-- 验证数据
SELECT 'Users created:' as info, COUNT(*) as count FROM users WHERE id >= 1001 AND id <= 1008;
SELECT 'Team players linked:' as info, COUNT(*) as count FROM team_players WHERE user_id >= 1001 AND user_id <= 1008;
SELECT 'Weekly reports:' as info, COUNT(*) as count FROM weekly_reports;
SELECT 'Weekly periods:' as info, COUNT(*) as count FROM weekly_report_periods;
SELECT 'Notifications:' as info, COUNT(*) as count FROM notifications WHERE user_id >= 1001 AND user_id <= 1008;
